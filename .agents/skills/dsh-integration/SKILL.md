...
- **`DSH_PROVIDER_API_KEY` secret'ı ayarlanana kadar workflow'lar "Check provider key is configured" adımında durur ve bilgilendirici bir yorum bırakıp `exit 0` ile başarıyla biter.** İlk sürümde bootstrap script secret boşken hiçbir yorum bırakmadan `exit 1` ile durup job'u kırmızı yapıyordu — bu, bu fork'un ilk gerçek CI koşumunda tam olarak böyle gözlemlendi (bkz. workflow run logu). Kök neden: bootstrap script'in kendisi (yerel/elle kullanım için doğru olan) fail-loud davranışını koruyor, ama workflow artık bootstrap'ı çağırmadan önce secret'ın varlığını ayrı bir adımda kontrol edip yokluğunda job'u erken, başarıyla sonlandırıyor. Repo sahibinin secret'ı eklemesi hâlâ gerekiyor; PR'ı hiçbir zaman bloklamadı (o job zaten "advisory"dir), ama artık job'un kendisi de kırmızı olmuyor.
- **Gerçek bir provider isteği ile uçtan uca bir koşum henüz gözlemlenmedi.** Script dallarının tümü (diff boyutu, prompt byte boyutu, `pr view`/`issue view` fetch hatası, timeout, dsh hatası/E2BIG, boş rapor, etiket süzme) stub'larla doğrulandı; gerçek yayınlanmış `dsh` paketi bu profili gerçekten compose edip bir provider isteği gerçekten denedi (bu makine ağ politikası yüzünden `ollama.com`'a erişemedi, ama `dsh`'nin kendisi doğru şekilde denedi ve doğru exit code/stderr ile başarısız oldu); ve secret henüz yokken bu fork'un kendi GitHub Actions'ında gerçek bir `pull_request` tetiklenmesi gözlemlendi (yukarıdaki madde). Secret eklendikten sonraki ilk gerçek başarılı koşum hâlâ gözlemlenmedi.
