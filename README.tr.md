...
Bu otomasyon iki şekilde doğrulandı: stub'lanmış `gh`/`dsh`'ye karşı inceleme ve triage script'lerinin her dalı (aşırı büyük diff, çekme hatası, timeout, provider hatası, boş yanıt, etiket filtreleme); ve gerçekten yayınlanmış `@deepseek-ai/dsh` paketinin bu tam profili başarıyla kurup birleştirmesi ve gerçek bir (kısıtlı bir sandbox'tan erişilemese de) provider isteği göndermesi. **Gerçek GitHub Actions içinde, erişilebilir bir provider'a karşı eksiksiz bir koşum henüz gözlemlenmedi** — ilk gerçek tetiklenme bootstrap'ta küçük bir ince ayar gerektirebilir; bir job'un ilk koşumu beklenmedik davranırsa Actions logunu kontrol et.
